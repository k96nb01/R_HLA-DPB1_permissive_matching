bin---
title: "SCT DPB1 permissiveness matching V2.0"
...
```{r}
#Calculate DPB1 permissiveness for DPB1 mismatches. Instead of hard-coding DPB1 permissiveness, the code uses the online DPB1 T-Cell Epitope Algorithm v2.0 tool hosted by the IMGT (https://www.ebi.ac.uk/ipd/imgt/hla/dpb_v2.html). While this is slower, as the code has to request and wait for responses from the web, the IMGT's tool is continually updated with new DPB1 alleles, and therefore should be future-proof. This code will determine if a DPB1 mismatch is permissive or non-permissive as long as the IMGT tool is working.
...
(DPB1_permisive_a <- BMT_cases_MM_matches 
   ...
   %>% mutate(TCE_result = str_extract(converted, '"tce_prediction".*'))
   %>% mutate(TCE_result = str_replace(TCE_result, '"tce_prediction" : ', ""))
   %>% mutate(TCE_result = str_replace_all(TCE_result, '"', ""))
   %>% mutate(Permissive = str_detect(TCE_result, "^Permissive"))
   )

#Recording GvH or HvG of the non-permissive mismatches was done separately so that the final table would only have values for DPB1 permissiveness if it was mismatched at DPB1, and then would only have values for GvH or HvG if it was a non-permissive mismatch.
...
#The "TCE_result" column is the result returned from the online IMGT TCE tool. "Permissive" translates this to a logical TRUE/FALSE value, and then the columns "NP_HvG," and "NP_GvH," have TRUE/FALSE values for the non-permissive mismatches, as appropriate.
```
